                    ...
                    <form action="{{ route('dashboard.clients.index') }}" method="get">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                ...
                                @if (auth()->check() && auth()->user()->hasPermission('clients_create'))
                            ...
                            <tbody>
                                @foreach ($clients as $index => $client)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $client->name }}</td>
                                        <td>{{ is_array($client->phone) ? implode('-', $client->phone) : $client->phone ?? 'N/A' }}
                                        </td>

                                        <td>{{ $client->address }}</td>
                                        <td>
                                            @if (auth()->check() && auth()->user()->hasPermission('orders_create'))
                                                <a href="{{ route('dashboard.clients.orders.create', $client->id) }}"
                                                    class="btn btn-primary btn-sm">@lang('site.add_order')</a>
                                            @else
                                                <a href="#"
                                                    class="btn btn-primary btn-sm disabled">@lang('site.add_order')</a>
                                            @endif
                                        </td>
                                        <td>
                                            @if (auth()->check() && auth()->user()->hasPermission('clients_update'))
                                            ...
                                            @endif
                                            @if (auth()->check() && auth()->user()->hasPermission('clients_delete'))
